...
export default function MobileBar({ city, variant }: { city: City; variant?: 'vyvod' | 'stacionar' | 'narkolog' | 'kodirovanie' | 'reabilitaciya' }) {
  const phoneCta =
    variant === 'vyvod' ||
    variant === 'stacionar' ||
    variant === 'narkolog' ||
    variant === 'kodirovanie' ||
    variant === 'reabilitaciya'
      ? 'Позвонить'
      : 'Позвонить сейчас'
  const barMod =
    variant === 'stacionar' || variant === 'narkolog' || variant === 'kodirovanie' || variant === 'reabilitaciya'
  ...
  return (
    <div className={`mobile-bar${barMod}`}>
      <div className="mobile-bar-inner">
        ...
        <button type="button" className="fab" onClick={openCallbackModal} aria-label="Обратный звонок">
