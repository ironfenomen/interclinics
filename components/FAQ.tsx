...
export default function FAQ({ city, variant }: { city: City; variant?: FaqVariant }) {
  ...
  if (variant === 'narkolog') {
    const items = [
      {
        q: 'Сколько стоит выезд нарколога на дом?',
        a: `Ориентир — от ${city.priceNarkolog.toLocaleString('ru')} ₽ за выезд. Итог — после осмотра, до продолжения лечения, с учётом объёма помощи. Детали первого контакта — у диспетчера.`,
      },
      {
        q: 'Как быстро приезжает врач?',
        a: `В ${city.namePrep} ориентир — около ${city.arrivalTime} минут. Фактическое время зависит от района, дороги и загрузки линии: при записи называют интервал и при необходимости обновляют.`,
      },
      {
        q: 'Что входит в выезд и что происходит на месте?',
        a: `Осмотр нарколога, оценка состояния, помощь на дому по показаниям — в том числе инфузионная и симптоматическая поддержка и рекомендации на ближайшее время. Объём задаёт только очный осмотр, не телефонный разговор.`,
      },
      {
        q: 'Как устроена конфиденциальность?',
        a: `Данные — по закону и политике; сведения о помощи в рамках врачебной тайны. Передача третьим лицам — только где это прямо допускает закон. Формат выезда обсудим на линии.`,
      },
      {
        q: 'Достаточно ли помощи на дому или нужен стационар?',
        a: `Решает врач после осмотра: на дому — когда это безопасно и по показаниям. При необходимости круглосуточного наблюдения или высокого риска обсудим госпитализацию и маршрут — без давления и без обещания, что «дома всегда достаточно».`,
      },
      {
        q: 'Кто приезжает на вызов?',
        a: `Врач‑нарколог; при необходимости состав дополняется по регламенту клиники. Детали — на линии под ваш адрес и ситуацию.`,
      },
      {
        q: 'Можно ли обратиться за взрослого близкого?',
        a: `Да, часто так и делают. Принудительного лечения взрослого без законных оснований нет; согласие на вмешательство — по действующим правилам. На линии помогут согласовать визит и разговор с близким.`,
      },
      {
        q: 'Что делать при очень тяжёлом состоянии?',
        a: `При прямой угрозе жизни — скорая помощь, 112. Если звоните к нам, диспетчер и врач оценят срочность: выезд нарколога или стационар — по ситуации.`,
      },
    ]
        ...
        <div className="c">
          <div className={styles.inner}>
            <header className={styles.header}>
              ...
              <p className={styles.subtitle}>
                Цена, срок, состав, конфиденциальность и когда уместен дом или стационар — до звонка.
              </p>
            </header>
  ...
  if (variant === 'kodirovanie') {
    const items = [
      {
        q: 'Когда можно делать кодирование?',
        a: `После снятия острой интоксикации и детоксикации по показаниям, при устойчивой трезвости — длительность периода оценивает врач. Кодирование «на следующий день после запоя» без осмотра не назначают: сначала осмотр и согласование плана.`,
      },
      {
        q: 'Сколько нужно быть трезвым перед процедурой?',
        a: `Часто ориентир — не менее 3–5 дней устойчивой трезвости; иначе бывает по показаниям. Точный срок зависит от динамики, сопутствующих факторов и метода — его фиксируют при очной консультации, а не «общими словами» по телефону.`,
      },
      {
        q: 'Как врач выбирает метод?',
        a: `По осмотру, анамнезу и целям: показания, риски, переносимость, ваше понимание режима после процедуры. Метод подбирают индивидуально, вместе с вами; без осмотра не выбирают «самый сильный» вариант.`,
      },
      {
        q: 'Чем отличается медикаментозное кодирование от психотерапевтического?',
        a: `В медикаментозных форматах опора на препараты и наблюдение после введения; в психотерапевтических — на работу с установками и мотивацией по методу. Иногда обсуждают комбинацию. Ни один формат не заменяет дальнейшую поддержку и образ жизни — это этап плана, а не разовое «нажатие кнопки».`,
      },
      {
        q: 'Есть ли противопоказания?',
        a: `Да, их определяет врач: тяжёлые соматические состояния, острые психические расстройства вне контроля, беременность и другие ситуации могут быть ограничением или поводом отложить процедуру. Нельзя перечислить все случаи в одном ответе — окончательное решение после очной оценки.`,
      },
      {
        q: 'Как долго действует кодирование?',
        a: `Зависит от метода: у имплантов и инъекций — ориентир по сроку действия препарата; у курсов и психотерапевтических форматов — иной горизонт и план наблюдения. Цифры на сайте — ориентир; детали и поведение при изменении состояния обсуждают с врачом при планировании.`,
      },
      {
        q: 'Можно ли записаться на консультацию для взрослого близкого?',
        a: `Да, так часто поступают. Принудительного лечения взрослого без законных оснований нет; согласие на медицинское вмешательство — в порядке, установленном законом. На линии подскажут, как согласовать визит и разговор о консультации по кодированию.`,
      },
      {
        q: 'Если сначала нужны детоксикация или стационар?',
        a: `Если безопаснее сначала стабилировать состояние в стационаре или пройти детоксикацию, врач объяснит порядок: сначала этот этап, затем при показаниях — обсуждение кодирования. Важна последовательная и безопасная помощь, а не «успеть закодировать любой ценой».`,
      },
    ]
        ...
        <div className="c">
          <div className={styles.inner}>
            <header className={styles.header}>
              ...
              <p className={styles.subtitle}>
                Условия и границы помощи в {city.namePrep} — по делу, без обещаний «универсального» результата.
              </p>
            </header>
